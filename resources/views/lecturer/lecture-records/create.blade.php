...
<body>
<div class="container">
    <a href="{{ route('lecturer.subject.lecture-records', $subject->id) }}" class="back-btn">
    ...
    <div class="card-box">
        <form action="{{ route('lecturer.subject.lecture-records.store', $subject->id) }}" method="POST">
            @csrf
            ...
            <div class="alert alert-info rounded-3 small">
                <i class="bi bi-info-circle"></i> Date, start time and end time will be scheduled by the admin.
            </div>

            <div class="mb-4">
                <label class="form-label">Content Covered</label>
                <textarea name="content_covered" class="form-control" rows="4" required></textarea>
            </div>
