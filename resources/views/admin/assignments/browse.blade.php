...
<style>
    * { box-sizing:border-box; }
    ...
    .faculty-header .icon-circle {
        width:38px; height:38px; border-radius:10px; background:#fef3c7; color:#f59e0b;
    ...
    .subject-link:hover { border-color:#f59e0b; box-shadow:0 4px 12px rgba(245,158,11,0.14); color:#012147; }
    .subject-link .subj-label { display:flex; align-items:center; gap:10px; font-size:13.5px; font-weight:500; }
    .subject-link .subj-icon {
        width:28px; height:28px; border-radius:8px; background:#fef3c7; color:#f59e0b;
                                        ...
                                        @foreach($semesters as $semesterName => $subjects)
                                            ...
                                            <div class="semester-body" id="sem-{{ $semId }}">
                                                @foreach($subjects as $subject)
                                                    <a href="{{ route('admin.subjects.assignments.index', $subject->id) }}" class="subject-link">
